...
int (*fpoint[])(int,int) = {&add, &sub, &mult, &div};
// performs the given operation on elements of the arrays
int* performOp(int *a, int *b, int size, int (*fpoint)(int,int)/* ADD FUNCTION POINTER PARAMETER HERE */);
...
int main(){
  ...
  while(choice != 5)
  {
	...
	result = performOp(array1,array2,size, fpoint[j]/* PASS IN FUNCTION POINTER FROM ARRAY[j] */);		
